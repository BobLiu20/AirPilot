/**
	...
	*{ TIM3, GPIOA, GPIO_Pin_7, TIM_Channel_2, TIM3_IRQn, 0, },   //2		 YAW	  
	*{ TIM3, GPIOB, GPIO_Pin_1, TIM_Channel_4, TIM3_IRQn, 0, },   //3		 THROTTLE
	*{ TIM3, GPIOA, GPIO_Pin_6, TIM_Channel_1, TIM3_IRQn, 0, },   //4		 AUX1
...
/* define ------------------------------------------------------------*/
/* variables ---------------------------------------------------------*/
static int16_t Rc_CaptureVal[6];
...
static void PWM_RcCaptureCallback(u8 ch,u16 capture)
{
	static u8 state[6] = {0,0,0,0,0,0};	//标记
	static u16 rise[6] = {0,0,0,0,0,0};	//记录上升沿时的值
	static u16 fall[6] = {0,0,0,0,0,0};	//记录下降沿时的值
	if(state[ch]==0)
	{
		rise[ch]=capture;
		state[ch]=1;
